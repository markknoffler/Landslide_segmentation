...
Table~\ref{tab:positioning} summarizes how PS-GPLNet relates to representative lines of work. The novelty claim is architectural closure: the same FS formalism at inlet, fusion, skips, decoder cells, and output equilibrium---implemented in \textit{from\_first\_steps/} and documented in \textit{model\_architecture.md}.
...
We compare against LinkNet, DEP-UNet, GMNet, RMAU-Net, TransUNet, EMR-HRNet, ShapeFormer, Dual-Stream UNet, U-Net, DeepLabV3+, and dual\_stream\_gated under the same ablation harness. TriEncoderCFMNet is excluded (unpublished). PS-GPLNet numbers are best validation epoch from \textit{outputs\_final\_bijie} and \textit{outputs\_final\_l4s}.

\subsection{Quantitative results}
Table~\ref{tab:results} reports best-epoch validation scores across both the Bijie and Landslide4Sense benchmarks. On Bijie, PS-GPLNet reaches F1\,$=$\,0.933 and IoU\,$=$\,0.907, surpassing DeepLabV3+ (0.927/0.899) and dual\_stream\_gated (0.897/0.869). On Landslide4Sense, PS-GPLNet achieves F1\,$=$\,0.736 and IoU\,$=$\,0.660, ahead of DeepLabV3+ (0.710/0.636) and the dual-stream variants in our ladder.

\begin{table*}[t]
\centering
\caption{Quantitative evaluation on the Bijie and Landslide4Sense benchmarks (best-epoch validation summary).}
\label{tab:results}
\setlength{\tabcolsep}{5pt}
\small
\begin{tabular}{l ccccc | ccccc}
\toprule
& \multicolumn{5}{c|}{\textbf{Bijie Benchmark}} & \multicolumn{5}{c}{\textbf{Landslide4Sense Benchmark}} \\
...
\midrule
\textbf{PS-GPLNet (ours)} & \textbf{0.991} & \textbf{0.956} & 0.946 & \textbf{0.933} & \textbf{0.907} & \textbf{0.986} & 0.802 & \textbf{0.788} & \textbf{0.736} & \textbf{0.660} \\
\bottomrule
\end{tabular}
\end{table*}
