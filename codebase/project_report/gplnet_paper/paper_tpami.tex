...
\label{sec:ablation}
Following the diagnostic organization used in strong multimodal ablations (Difficult vs.\ Easy stress tests; component matrix; mechanism swaps), we treat \textbf{Landslide4Sense as the hard regime} (cross-region scars, severe imbalance) and \textbf{Bijie as the easier regime} (larger polygons, more homogeneous acquisition). Absolute Full-model numbers in Table~\ref{tab:ablation-main} are taken from the primary-server runs already reported in Table~\ref{tab:results} (Bijie F1/IoU \(0.933\)/$0.907$; L4S \(0.736\)/$0.660$)---\emph{not} from the compact ablation Full checkpoint. Component rows are matched-protocol removals under a compact low-VRAM schedule (EfficientNet-B0, \(C{=}32\), LoRA \(r{\le}4\), FP32, batch~$2$, $40$ epochs, seed~$42$), so that every module is compared under identical compute. Protocol-matched \(\Delta\) relative to the compact Full reference (Bijie F1~$0.804$; L4S F1~$0.693$) are reported in Table~\ref{tab:ablation-delta} and Figures~\ref{fig:abl_f1}--\ref{fig:abl_rank}.

\paragraph{What each removal tests.}
\texttt{w/o FS-gate} disables Taylor-stabilized FS gating in mechanistic cells (identity residual). \texttt{w/o MPEF} replaces instability routing by mean fusion of dual logits. \texttt{w/o CMB} bypasses Complementary Modality Bridge (CNN/physics hybrids). \texttt{w/o MAO} replaces GeoEGCA bottleneck fusion by a plain $1{\times}1$ concat mix. \texttt{w/o TTEB} replaces the skip lattice by bilinear identity skips. \texttt{w/o Prithvi} drops the foundation stream. \texttt{Path-A only} keeps a single RGB-proxy decode path (no DEM path / no MPEF competition).

% Force ablation tables into the reading stream (not deferred past later figures).
\onecolumn
\vspace{0.4em}
{\centering
\captionof{table}{Ablation study: component effectiveness on Difficult (Landslide4Sense) and Easy (Bijie) stress tests. Full row $=$ primary-server PS-GPLNet (Table~\ref{tab:results}). Other rows $=$ matched compact protocol (best val epoch). Acc/Prec/Rec/F1/IoU.}
...
\centering
\captionof{table}{Protocol-matched \(\Delta\)F1 / \(\Delta\)IoU vs.\ compact Full (Bijie $0.804$/$0.773$; L4S $0.693$/$0.617$). Negative $=$ removal hurts.}
...
Removed & \(\Delta\)F1 & \(\Delta\)IoU & \(\Delta\)F1 & \(\Delta\)IoU \\
\midrule
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\captionof{table}{Mechanism-swap focus: FS gating, MPEF, and dual-path competition (matched compact F1).}
\label{tab:ablation-mech}
\setlength{\tabcolsep}{5pt}
\small
\begin{tabular}{lcc}
\toprule
Mechanism setting & L4S (hard) & Bijie (easy) \\
\midrule
Full (compact ref.) & 0.693 & 0.804 \\
w/o FS-gate & 0.703 & 0.896 \\
w/o MPEF (mean fuse) & 0.689 & 0.812 \\
...
\caption{Protocol-matched \(\Delta\)F1 vs.\ compact Full. Negative bars: module helps Full under the matched budget.}
\label{fig:abl_delta}
\end{figure}
...
\caption{Component ablation IoU with primary Full as dashed references.}
\label{fig:abl_iou}
\end{figure}
...
\caption{Signed module contribution \(\mathrm{Full_{compact}}-\mathrm{Variant}\) (F1). Color: positive help.}
\label{fig:abl_help}
\end{figure}
...
\caption{Mechanism swaps: FS gating, MPEF, and Path-A only.}
\label{fig:abl_mech}
\end{figure}
...
\caption{Cross-dataset ablation ranking. Star: primary-server Full.}
\label{fig:abl_rank}
\end{figure}
