"""Manifold-Aligned Orthogonal Geo-Equilibrium Gated Cross-Attention (MAO-GeoEGCA)."""
...
class MAOGeoEGCA(nn.Module):
    """
    Bottleneck fusion from architecture_chain_of_thought.md / model_architecture.md (fusion=mao).

    Physics anchor: Conv1x1([x_rgb; x_dem])
    Q from physics anchor; K,V from foundation model (Prithvi)
    Equilibrium gate from element-wise (t_fm * x_physics)
    Manifold alignment: K' = K * normalize(Q)
    """
