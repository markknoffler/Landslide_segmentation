...
class SingleEncoderDualDecoderNet(nn.Module):
    """
    New model architecture with single encoder and dual decoders.

    This model uses a single encoder that processes both input tensors,
    followed by two separate decoders that process the encoder features independently.
    The outputs from both decoders are then combined using gate fusion modules.
    """
    def __init__(
        self,
        n_classes: int = 1,
        ...
        # Single encoder for both inputs
        ...
        # Create two decoders with shared encoder
    ...
    def forward(
        ...
        # Unpack features
        a1, a2, a3, a4, a5 = a_feats
        b1, b2, b3, b4, b5 = b_feats

        # Gate fusion of encoder features
        f4, reg_c4 = self.efuse_c4(a4, b4)
        f3, reg_c3 = self.efuse_c3(a3, b3)

        # Process through separate decoders
        _, _, x3a, x4a = self.decoderA(a1, a2, f3, f4, a5)
        _, _, x3b, x4b = self.decoderB(b1, b2, b3, b4, b5)

        # Gate fusion of decoder outputs
...
DiGATe_Unet = SingleEncoderDualDecoderNet